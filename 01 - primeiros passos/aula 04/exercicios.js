...
const my3FavoriteTVShows = ['Um maluco no pedaço', 'Todo mundo odeia o Chris', 'Kenan e Kel']
...
const characterLimit = 39
const resultMessage = `A string que a "typeSentence" armazena tem mais de ${characterLimit} caracteres? ${String(typeSentence.length > characterLimit)}.`
...
const sentenceFalsyValues = `Todos os ${falsyValues.length} itens do array "falsyValues" são falsy. Inclusive o ${falsyValues[2]}.`
console.log(sentenceFalsyValues)
...
console.log(crazyConversion) // o valor é true porque o valor da crazyOperantion é 1.
...
const sentenceSumAges = `A soma entre o 1º e o 3º item de "ages" é menor ou igual a 92. Essa afirmação é: ${agesSum === 92}.`
console.log(sentenceSumAges)
...
const hasEight = evenNumbers.some(number => number === 8)
console.log(hasEight)

/** OO método .some() no JavaScript verifica se pelo menos um dos elementos de 
 * um array satisfaz uma condição (retorna true em uma função de teste). 
 * Se encontrar um único valor que atenda a essa condição, ele para e retorna 
 * true. Se nenhum elemento atender, retorna false.  */